...
class Net3(nn.Module):
    ...
    def forward(self, x, edge_index, batch):
        ...
        x_pooled1 = self.gc3(x, edge_index)
        x_pooled1 = torch.relu(x_pooled1)

        x_pooled1 = self.gc4(x_pooled1, edge_index)
        x_pooled1 = torch.relu(x_pooled1)


        x_pooled2 = self.gc5(x_pooled1, edge_index)
        x_pooled2 = torch.relu(x_pooled2)

        readout = torch.cat([x_pooled2[batch == i].mean(0).unsqueeze(0) for i in torch.unique(batch)],
                            dim=0)
