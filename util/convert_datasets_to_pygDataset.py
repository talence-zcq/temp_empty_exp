...
def my_load_Hyper_data(args, data_name, feature_noise):
    ...
    if data_name in existing_dataset:
        if (feature_noise is not None) and data_name in synthetic_list:
            p2raw = './data/AllSet_all_raw_data/'
            dataset = dataset_Hypergraph(name=data_name,
                                         feature_noise=feature_noise,
                                         p2raw=p2raw)
        else:
            if data_name in ['cora', 'citeseer','pubmed']:
                p2raw = './data/AllSet_all_raw_data/cocitation/'
            elif data_name in ['coauthor_cora', 'coauthor_dblp']:
                p2raw = './data/AllSet_all_raw_data/coauthorship/'
            elif data_name in ['yelp']:
                p2raw = './data/AllSet_all_raw_data/yelp/'
            else:
                p2raw = './data/AllSet_all_raw_data/'
            dataset = dataset_Hypergraph(name=data_name, root='./data/pyg_data/hypergraph_dataset_updated/',
                                         p2raw=p2raw)
        return dataset
